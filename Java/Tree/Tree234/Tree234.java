...
class Tree234 {
	...
	public void insert(double dValue) {
		...
		while (true) {
			...
			else if (curNode.isLeaf()) // if node isleaf,
				break; // go insert
				// node is not full, not a leaf; so go to lower level
			else
				curNode = getNextChild(curNode, dValue);
		} // end while
		curNode.insertItem(tempItem); // insert newDataItem
	} // end insert()
		// ------------------------------------------------------------
	...
	public Node getNextChild(Node theNode, double theValue) {
		...
		for (j = 0; j < numItems; j++) // for each item i node
		{ // are we less?
	...
	private void recDisplayTree(Node thisNode, int level, int childNumber) {
		...
		thisNode.displayNode(); // display thi node
